  ...
  <div>
    <div class="ayaid inline-block">
      ...
      <el-popover :disabled="disabled" popper-class="inputtishi" ref="popover" placement="bottom-start" v-model="visible" trigger="hover"
        ...
        <el-cascader slot="reference" v-model="initValue" placeholder="试试搜索：三庄镇" :options="xuanze" filterable
          change-on-select clearable @change="xuanzeChange" :elCascaderClass="elCascaderClass"></el-cascader>
      </el-popover>
...
<script>
  ...
  export default {
    ...
    created() {
      this.$store.dispatch('quYu_Ac')
    },
    computed: {
      xuanze() {
        return this.$store.state.quYu
      },
      initValue: {
        get() {
          return this.$store.state.duoxuan[3].initValue
        },
        set(value) {
      ...
      initLabel: {
        get() {
          return this.$store.state.duoxuan[3].initLabel
        },
        set(value) {
      ...
      searchValue() {
        return this.$store.state.duoxuan[3].searchValue
      }
    },
    watch: {
      searchValue() {
    ...
    methods: {
      xuanzeChange(zuobiao, string) {
       ...
       // this.shaixuan(this.$store.state.quYu, zuobiao[zuobiao.length - 1])
       // xiugaitijiao(this.$store);
      },
      shaixuan(shuzu, id) {
        for (let i = 0; i < shuzu.length; i++) {
          this.shaixuan(shuzu[i], id)
          if (shuzu[i].value == id) {
            shuzu[i].disabled = true;
            return
          }
        }
      },
      addshaixuan() {
        if (this.$store.state.duoxuan[3].searchValue.length == 0) {
          this.$message.error('请选择第一个筛选条件');
          return;
        } else if (this.$store.state.duoxuan[3].eq == this.$store.state.duoxuan[3].searchValue.length) {
          this.$message.error('请选择一个当前的筛选条件');
          return;
        } else if (this.$store.state.duoxuan[3].searchValue[this.$store.state.duoxuan[3].eq].length == 0) {
          this.$message.error('点击添加条件后请选择新的筛选条件');
          return;
        } else {
          this.$message.success('点击添加条件后请选择新的筛选条件');
          var value = ++this.$store.state.duoxuan[3].eq;
          this.initValue = [];
          this.initLabel = [];
